...
- **Event Trigger** - The interaction of event start
  ...
  - **Gesture** - A gesture is detected on a widget with the selected direction
  - **Short clicked** - short click is detected
  - **Key** - A key is sent to the widget
  - **Edited** - Triggered when any part of a text object's content is changed
  - **Insert** - Fires when new text is inserted into a text input object
...
Increment or decrement the value of the selected spinbox.

- **Target** - The spinbox you want to change
- **Direction** - You can choose to increase or decrease the value

## Switch Theme

From the dropdown menu, you can select which global theme to switch to. When this happens, it refreshes the global colors on all widgets of all screens, including widgets created from code.